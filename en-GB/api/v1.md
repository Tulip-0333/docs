...
`privileges`  | `uint64` | Privileges of the token, as defined in [Privileges](overview#privileges). | No (defaults 0)
...
**[Implements pagination](overview#pagination).**
...
Retrieves information about one or more users.
...
If either `id` or `name` is passed, then the result will be embedded in the global response object. If neither of those two are passed, the users returned will be in the array `users`. Of course, the user(s) to be returned will be of type [User](types#user).
...
Retrieves an index of Ripple's documentation files. **[Implements pagination](overview#pagination).**
...
[IN Parameters]: overview#in-parameters
